...
def notadebdecompracabinf_listar(request):
    ...
    notadebdecompracab = Notadebitocompcab.objects.all()
    if idproveedor == '':
        idproveedor = 0
    if notadebdecompracab.exists() and int(idproveedor) >= 1:
        notadebdecompracab=notadebdecompracab.filter(idproveedor__contains=idproveedor)
    if notadebdecompracab.exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        notadebdecompracab= notadebdecompracab.filter(fecha__gte=fecha_inicio)
    if notadebdecompracab.exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        notadebdecompracab= notadebdecompracab.filter(fecha__lte=fecha_fin)
    if notadebdecompracab.exists() and tipodoc:
        notadebdecompracab=notadebdecompracab.filter(tipodoc=tipodoc)
