...
def ordendecompracabinf_listar(request):
    ...
    ordendecompracab = Ordencompcab.objects.all()
    if idproveedor == '':
        idproveedor = 0
    if ordendecompracab.exists() and int(idproveedor) >= 1:
        ordendecompracab   = ordendecompracab .filter(idproveedor__contains=idproveedor)
    if ordendecompracab .exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        ordendecompracab   = ordendecompracab .filter(fecha__gte=fecha_inicio)
    if ordendecompracab .exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        ordendecompracab   = ordendecompracab .filter(fecha__lte=fecha_fin)
    if ordendecompracab .exists() and tipodoc:
        ordendecompracab   = ordendecompracab .filter(tipodoc=tipodoc)
