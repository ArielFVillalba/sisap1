...
def notacreditodeventacabinf_listar(request):
    ...
    notacreditodeventacab = Notacreditoventacab.objects.all()
    if idcliente == '':
        idcliente = 0
    if notacreditodeventacab.exists() and int(idcliente) >= 1:
        notacreditodeventacab = notacreditodeventacab .filter(idcliente__contains=idcliente)
    if notacreditodeventacab.exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        notacreditodeventacab= notacreditodeventacab .filter(fecha__gte=fecha_inicio)
    if notacreditodeventacab.exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        notacreditodeventacab= notacreditodeventacab .filter(fecha__lte=fecha_fin)
    if notacreditodeventacab.exists() and tipodoc:
        notacreditodeventacab= notacreditodeventacab .filter(tipodoc=tipodoc)
